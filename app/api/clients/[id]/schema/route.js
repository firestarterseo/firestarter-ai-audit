...
// (existing partial schema, tel: links, social/profile links) for any
// field not already saved on the client record -- a strategist should
// review and confirm, not type these in from a blank form when the site
// already has the answer sitting in its own markup. `suggested` lists
// exactly which fields this call filled in (vs. what was already saved),
// so the UI can label them as auto-detected rather than presenting them as
// if the strategist had entered them. Never overrides a value that's
// already set, including one deliberately left blank on purpose after a
// prior save.
