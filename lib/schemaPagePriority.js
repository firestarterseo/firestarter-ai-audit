...
// `excludePaths` (Set<path>, default none) -- added 2026-09-02 for the
// Schema recommendation/queue workflow correction. This is how PRODUCT
// DECISION #3 ("completed/passing pages must not be recommended") and
// #11 ("exclude pages with states NO_ACTION_NEEDED, COMPLETED, dismissed...
// then recompute next 10 eligible recommendations") are actually enforced:
// a path in `excludePaths` is skipped entirely while walking toward
// targetMax, exactly like a LOW_PRIORITY/OTHER tier is skipped today. It is
// NOT removed from `all` -- the full discovered universe must stay
// available for the "All discovered pages" view (PRODUCT DECISION #1: the
// full universe is kept internally for classification/prioritization, only
// hidden from the DEFAULT recommendation batch).
//
// This is also what makes "NEXT BEST 10, not TOP 10 FOREVER" (PRODUCT
// DECISION #2) require no batch-index/pagination state at all: `sorted`'s
// order never changes run to run (same deterministic tier/match/path
// ordering as before), so once a page's state moves it into
// `excludePaths`, the very next call to computeRecommendedSet naturally
// admits the next-best still-eligible page into the same targetMax-sized
// walk -- there is no "batch 1 vs batch 2" concept to track separately.
function computeRecommendedSet(pages, clientProfile = {}, { targetMin = 10, targetMax = 15, excludePaths = null } = {}) {
  ...
  for (const dossier of sorted) {
    ...
    if (excludePaths && excludePaths.has(dossier.path)) continue
