...
const { checkEntityCitationAuthority } = require('./checkers/entity-citation-authority-checker')
...
const { annotateWithSerpLandscape } = require('./serpLandscape')
const { annotateWithKeywordDifficulty } = require('./keywordDifficulty')
...
// runEntityCitationAuthorityPillar(client) -> pillar output contract.
// A fresh, targeted query against ai_visibility_tracked_runs (same table
// AI & GEO Visibility and Competitive Position already read) rather than
// threading rows through from either of those -- same "a free Supabase
// read isn't worth coupling pillars over" reasoning as
// runCompetitivePositionPillar's own comment on this. Needs `sentiment`
// specifically, which Competitive Position's own query doesn't select, so
// that one can't just be reused here. Never throws on its own (both
// checkEntityCitationAuthority and getAuthorityBacklinks degrade to empty/
// null on failure rather than throwing) -- still guarded by the caller's
// .catch() below since the Supabase read itself can fail.
async function runEntityCitationAuthorityPillar(client) {
  const supabase = getSupabaseServerClient()
  const { data: rows, error } = await supabase
    .from('ai_visibility_tracked_runs')
    .select('brand_mentioned, sentiment, raw')
    .eq('client_id', client.id)
    .order('run_at', { ascending: false })
    .limit(500)
  if (error) throw error

  return checkEntityCitationAuthority(client, {
...
async function runAudit(client, { triggerSource = 'manual' } = {}) {
  ...
  const [schemaResult, entityCitationAuthorityResult, technicalResult, contentResult, aiVisibilityResult, competitivePositionResult] = await Promise.all([
    fetchError
      ...
      : checkSchemaAndStructure(homepageHtml, { url: client.url }),
    runEntityCitationAuthorityPillar(client).catch(err => ({
      grade: null,
      score: null,
      noData: true,
      finding: 'Could not evaluate Entity & Citation Authority this run.',
  ...
  const pillars = [
    ...
    { pillar: 'entity_citation_authority', result: entityCitationAuthorityResult },
  ...
  if (competitivePositionResult?._raw?.keywordOpportunitiesChecked) {
    // Real SERP-landscape evidence (wired in 2026-08-15, see
    // lib/serpLandscape.js) -- runs BEFORE the LLM refinement below so it
    // has real, live Google-SERP top domains per keyword to reason from,
    // instead of inferring everything from the tracked competitor's brand
    // name alone. Reuses Cloro's "google" engine, the SAME API/key already
    // paid for and called weekly for AI & GEO Visibility -- one additional
    // live call per candidate keyword (capped at 15), confirmed with
    // Skyler as a worthwhile ongoing cost for real evidence over a guess.
    // Degrades gracefully (returns the original list, each item just
    // missing serpTopDomains/serpChecked) if CLORO_API_KEY isn't
    // configured or the call fails -- never blocks the audit run.
    const opportunitiesWithSerpData = await annotateWithSerpLandscape(
      competitivePositionResult._raw.keywordOpportunities,
      { apiKey: process.env.CLORO_API_KEY }
    ).catch(() => competitivePositionResult._raw.keywordOpportunities)

    // Real Ahrefs Keyword Difficulty (added 2026-08-15, see
    // lib/keywordDifficulty.js) -- one batched Ahrefs call for the whole
    // candidate list. Direct answer to a fair complaint that
    // realistic_tier was still just an LLM's read of domain names even
    // after the SERP-landscape/domain-rating fixes above: this gives
    // lib/keywordRelevance.js real KD numbers to compute the tier from
    // instead of guessing. Degrades gracefully (every candidate's
    // keywordDifficulty stays null) if AHREFS_API_KEY is missing or the
    // call fails -- refineKeywordOpportunities falls back to the LLM's
    // own judgment in that case, never blocks the audit run.
    const opportunitiesWithKeywordDifficulty = await annotateWithKeywordDifficulty(
      opportunitiesWithSerpData,
      { apiKey: process.env.AHREFS_API_KEY }
    ).catch(() => opportunitiesWithSerpData)
    ...
    const refinedOpportunities = await refineKeywordOpportunities(
      opportunitiesWithKeywordDifficulty,
      {
    ...
    ).catch(() => opportunitiesWithKeywordDifficulty)
