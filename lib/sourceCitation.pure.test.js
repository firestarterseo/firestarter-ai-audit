...
// Correction test 13 (added Phase 2A, 2026-09-01, alongside the
// lib/webPageFetch.js migration of inspectCitedUrl's transport mechanics):
// a robots.txt disallow rule must still produce 'unverifiable' (never
// 'verified_absent'), and the page itself must never be fetched once
// robots denies it. Closes a real coverage gap -- every existing mock
// fetcher in this file returned 404 for robots.txt (the fail-open/allowed
// path), so 'robots_blocked' was never actually exercised before this.
async function correctionTest13_robotsBlockedIsUnverifiableAndNeverFetchesThePage() {
  const fetcher = async (url) => {
    if (url.endsWith('/robots.txt')) return { ok: true, status: 200, text: async () => 'User-agent: *\nDisallow: /top-denver-seo\n' }
    throw new Error('inspectCitedUrl must never fetch the page itself once robots.txt disallows its path')
  }
  const inspection = await inspectCitedUrl('https://clutch.co/top-denver-seo-2', { name: 'Firestarter SEO' }, { fetcher })
  assert.strictEqual(inspection.verificationStatus, 'unverifiable')
  assert.strictEqual(inspection.failureReason, 'robots_blocked')
  assert.notStrictEqual(inspection.verificationStatus, 'verified_absent')
  log('CORRECTION TEST 13 (a robots.txt Disallow rule produces "unverifiable"/"robots_blocked" and the page itself is never fetched) PASSED')
}
...
async function main() {
  ...
  await correctionTest13_robotsBlockedIsUnverifiableAndNeverFetchesThePage()

  log('\nAll sourceCitation pure-function tests passed (no DB, no LLM required), including all 12 evidence-strength-correction tests plus a Phase 2A robots-blocked regression test.')
