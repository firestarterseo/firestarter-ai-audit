...
function run() {
  ...
  // 14. CALLER MUST DERIVE THE EXCLUSION KEY FROM THE REAL DOSSIER PATH, NOT
  // A HARDCODED ASSUMPTION -- regression test for the 2026-09-02 live bug
  // where SchemaWizard.js hardcoded '/' as "the homepage's path" instead of
  // reading it off the actual classified page. This test proves the
  // UNDERLYING exclusion mechanism itself is agnostic to what the
  // homepage's real path string is -- it is the CALLER's job (see
  // SchemaWizard.js's `homepageEntry` fix) to find "the page classified as
  // Home" in its own data and key excludePaths by THAT page's real `path`,
  // never a literal. Simulates a (defensive, not expected-real-world, but
  // structurally possible) case where the Home-classified page's path is
  // NOT the literal string '/'.
  {
    const pages = [
      page({ path: '/index', type: 'Home' }), // deliberately NOT '/' -- the point of this test
      page({ path: '/services/seo/', type: 'Service' })
    ]
    // The WRONG pattern (what the live bug did): hardcode '/'.
    const wrongExclude = new Set(['/'])
    const wrongResult = computeRecommendedSet(pages, {}, { targetMin: 10, targetMax: 10, excludePaths: wrongExclude })
    assert.ok(wrongResult.recommended.some(d => d.path === '/index'), 'sanity check: hardcoding the wrong path must fail to exclude the real homepage (reproduces the live bug)')

    // The RIGHT pattern: derive the key from the actual Home-typed page.
    const homepageEntry = pages.find(p => p.type === 'Home')
    const rightExclude = new Set([homepageEntry.path])
    const rightResult = computeRecommendedSet(pages, {}, { targetMin: 10, targetMax: 10, excludePaths: rightExclude })
    assert.ok(!rightResult.recommended.some(d => d.path === '/index'), 'deriving the exclusion key from the real classified Home page must exclude it regardless of its actual path string')
    assert.deepStrictEqual(rightResult.recommended.map(d => d.path), ['/services/seo/'])
    log('TEST 14 (regression: the exclusion key must be derived from the real classified Home page\'s own path, never a hardcoded literal -- reproduces and proves the fix for the 2026-09-02 live homepage-still-recommended bug) PASSED')
  }
