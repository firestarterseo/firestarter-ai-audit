...
// signal, not re-derived. Deliberately conservative: match_confidence is
// always 'low' here, since this is a pattern fallback, not real judgment.
function heuristicPageFit(candidate, intent, index) {
  ...
  const looksHowTo = HOWTO_SLUG_PATTERN.test(path) || type === 'Article' || type === 'Case Study'
  const looksPricing = PRICING_SLUG_PATTERN.test(path)
  const looksComparison = COMPARISON_SLUG_PATTERN.test(path)
...
async function evaluatePageIntentMatch({ promptText, competitorPage, candidates, apiKey } = {}) {
  ...
  function fallback(errorMessage) {
    ...
    const competitorEval = competitorPage
      ? heuristicPageFit({ url: competitorPage.url, path: safePathFromUrl(competitorPage.url) }, heuristic.intent, -1)
      : null
    return {
  ...
  const payload = {
    ...
    client_candidates: candidates.map((c, i) => ({ index: i, url: c.url, path: c.path || safePathFromUrl(c.url), page_type_classification: c.type || null, source: c.source }))
  }
