...
const { getOrganicCompetitors, getDomainMetrics, getDomainRating, getOrganicKeywords } = require('./checkers/ahrefs')
const { isNonCompetitorDomain, hostnameOf, normalizeDomain } = require('./nonCompetitorDomains')

const MAX_TRACKED_ROWS = 500

// --- Tracked-competitor list trimming logic (added 2026-08-15) ---
// Direct client feedback surfaced two related problems with the
// auto-detected competitor list: (1) "will this hold up for a home
// services business or a divorce attorney?" (see
// MULTI_INDUSTRY_DIRECTORY_DOMAINS in nonCompetitorDomains.js for that
// half of the fix) and (2) "25 seems like a lot of competitors for this
// list, any way to filter some out?" -- because, as written before this
// change, there was NO cap, NO minimum-occurrence threshold, and NO
// staleness decay anywhere in detectAndSyncCompetitors: a domain became a
// permanent tracked competitor the moment it was cited even ONCE across up
// to 500 historical AI-visibility runs, and nothing ever removed it again
// short of a strategist manually deactivating the row.
//
// Two independent fixes, used together rather than a single hard cap:
//   1. MIN_AI_CITATION_OCCURRENCES gates NEW ai_citation-sourced
//      candidates at insertion time -- a domain has to show up in at
//      least this many separate tracked runs (out of the most recent
//      MAX_TRACKED_ROWS) before it's promoted to a tracked competitor, not
//      just a single one-off mention. This uses detectFromAiCitations'
//      own `occurrences` count, which was already being computed every
//      run and then thrown away after only being used for the "looks like
//      a roundup" detectionNote -- a ready-made signal, just never
//      actually gated on before now. Ahrefs-sourced candidates are exempt
//      (see meetsTrackingBar below) because that signal is fundamentally
//      different: getOrganicCompetitors is a LIVE call reflecting CURRENT
//      keyword overlap, not a count of historical mentions, so "appeared
//      once" already means "is a real competitor right now," not "was
//      mentioned once, maybe a fluke."
//   2. STALE_AFTER_DAYS soft-deactivates (active=false, reversible --
//      NOT a hard delete like pruneStaleCompetitors) any existing,
//      non-manual, non-strategist-confirmed tracked competitor that
//      hasn't been genuinely re-detected in a long time. This directly
//      answers "25 seems like a lot" for lists that already grew large
//      before this fix shipped, not just new detections going forward.
//      A hard numeric cap (e.g. "only ever track the top 10") was
//      considered and rejected: it truncates by an arbitrary rule
//      (biggest/most-recent N) rather than by whether a domain is still
//      actually a real, current competitor, and it would silently hide a
//      genuine small-but-persistent competitor just because a client
//      happens to have 11 real ones. Occurrence-threshold + staleness-decay
//      both fail toward "still tracked, needs a human look" rather than
//      toward "silently dropped/truncated," which fits this project's
//      "verify, don't guess" philosophy better than a blunt cap does.
const MIN_AI_CITATION_OCCURRENCES = 2
...
async function detectFromAiCitations(client) {
  ...
  const domainStats = new Map() // domain -> { occurrences, roundupRowCount, lastCitedAt }
  ...
  ;(rows || []).forEach(row => {
    ...
    rowDomains.forEach(d => {
      const entry = domainStats.get(d) || { occurrences: 0, roundupRowCount: 0, lastCitedAt: null }
      entry.occurrences++
      if (rowDomains.size >= 3) entry.roundupRowCount++
      // Track the MOST RECENT run that actually cited this domain --
      // rows are already read newest-first, so in practice the first
      // occurrence processed already carries the newest run_at, but this
      // explicit max-comparison doesn't rely on that ordering assumption
      // holding forever. This is the real per-domain "last cited" date
      // used by deactivateStaleCompetitors below -- NOT the same thing as
      // "was this domain found in today's scan," since a single old
      // citation keeps showing up in every scan for as long as it stays
      // inside the MAX_TRACKED_ROWS window.
      if (runAt && (!entry.lastCitedAt || new Date(runAt) > new Date(entry.lastCitedAt))) {
        entry.lastCitedAt = runAt
      }
...
// deactivateStaleCompetitors(client, supabase) -> Promise<number> (rows deactivated)
// The staleness half of the trimming logic described above pruneStaleCompetitors's
// header this file also has. Soft-deactivates (active=false, reversible via the
// existing "Reactivate" button in CompetitorsManager.js) any tracked competitor
// that:
//   - is currently active,
//   - was NOT added by hand (source != 'manual'), and
//   - has NOT been explicitly confirmed by a strategist (confirmed_at is null --
//     re-adding/confirming an auto-detected row through the manual-add form sets
//     this even though source itself stays 'ai_citation'/'ahrefs', and that
//     explicit human sign-off should exempt it from auto-decay the same way a
//     fully manual row is exempt),
// AND whose true recency has fallen outside STALE_AFTER_DAYS:
//   - ai_citation-sourced rows use last_cited_at (the actual last-citation date,
//     see detectFromAiCitations above) -- last_seen_at is NOT reliable for this
//     source, since a single old citation keeps re-appearing in every scan for as
//     long as it's inside the MAX_TRACKED_ROWS window, which would make a
//     last-seen-based check never actually fire.
//   - every other source (today, just 'ahrefs') uses last_seen_at, which IS a
//     reliable recency signal there -- getOrganicCompetitors is a live call
//     reflecting current keyword overlap, so a row simply stops getting touched
//     the moment it stops showing up, with no rolling-window artifact to correct
//     for.
// A row with no recency value at all (shouldn't normally happen post-migration,
// but possible for a row that predates this column) is treated as stale rather
// than skipped -- there's no evidence it's still current, so it should surface
// for a human look rather than sit untouched indefinitely.
async function deactivateStaleCompetitors(client, supabase) {
  const { data: rows, error } = await supabase
    .from('client_competitors')
    .select('id, domain, source, last_seen_at, last_cited_at')
    .eq('client_id', client.id)
    .eq('active', true)
    .neq('source', 'manual')
  ...
  const toDeactivate = (rows || []).filter(r => {
    const recency = r.source === 'ai_citation' ? r.last_cited_at : r.last_seen_at
    if (!recency) return true
    return new Date(recency).getTime() < cutoffMs
  })
  ...
  for (const row of toDeactivate) {
    const { error: updateError } = await supabase
      ...
      .update({
        active: false,
        detection_note: `Auto-deactivated: not ${row.source === 'ai_citation' ? 'cited' : 'seen'} again in over ${STALE_AFTER_DAYS} days. Reactivate below if this is still a real competitor.`
      })
      .eq('id', row.id)
...
async function detectAndSyncCompetitors(client, { ahrefsApiKey } = {}) {
  ...
  // Trimming fix #1 (see MIN_AI_CITATION_OCCURRENCES's header above): a
  // brand-new ai_citation-sourced candidate only gets PROMOTED to a
  // tracked competitor once it's cleared the minimum-occurrence bar.
  // Ahrefs-sourced candidates are exempt -- see the same header for why
  // that signal doesn't need (or benefit from) an occurrence count.
  // Sub-threshold candidates aren't lost: they simply aren't inserted yet,
  // and will be reconsidered fresh on every future run as their real
  // occurrence count changes.
  const meetsTrackingBar = c => c.source !== 'ai_citation' || c.occurrences >= MIN_AI_CITATION_OCCURRENCES
  ...
  if (candidates.length > 0) {
    ...
    const toInsert = candidates
      .filter(c => !existingDomains.has(c.domain) && meetsTrackingBar(c))
    ...
    // last_seen_at, last_cited_at, and detection_note all get refreshed
    // every run -- detection_note is derived evidence, not a strategist's
    // own input, so keeping it current (including clearing it back to
    // null if a domain stops looking like a roundup source) is more
    // useful than leaving it stuck at whatever it said the first time.
    // last_cited_at is only meaningful for ai_citation-sourced rows (see
    // its own header) -- omitted from the update for any other source so
    // it isn't stamped with a value that doesn't mean anything for that
    // row. This has to be a per-domain update (not one bulk
    // .update().in(...)) since each domain can have a different note;
    // source/name/confirmed_at/active are deliberately left untouched
    // here, same as before.
    const now = new Date().toISOString()
    const touchErrors = (await Promise.all(candidates.map(c => {
      const updates = { last_seen_at: now, detection_note: c.detectionNote || null }
      if (c.source === 'ai_citation') updates.last_cited_at = c.lastCitedAt || null
      return supabase
        .from('client_competitors')
        .update(updates)
        .eq('client_id', client.id)
  ...
  // Trimming fix #2: soft-deactivate whatever has genuinely gone stale,
  // using the just-refreshed last_seen_at/last_cited_at values above. Runs
  // every sync regardless of whether this run found any candidates at all
  // -- staleness is about a row's OWN historical timestamps, not about
  // this run's success, and STALE_AFTER_DAYS (90) is generous enough that
  // one failed/empty detection run can't itself cause a false
  // deactivation.
  await deactivateStaleCompetitors(client, supabase)
...
//
// Never throws -- getDomainMetrics/getDomainRating themselves never throw
// (see their own headers), so this only needs to await the concurrent
// calls.
//
// BUG FOUND AND FIXED 2026-08-15: this used to read domainRating off
// getDomainMetrics' result -- that field never actually came back (see
// checkers/ahrefs.js's getDomainMetrics header for the full story: it was
// requesting `domain_rating` from an Ahrefs report that doesn't have that
// field at all), so clientDomainRating was silently `null` on every audit
// run since the scale-matching fix shipped, and
// selectScaleComparableCompetitors below permanently fell back to its
// "no domain rating" path -- comparing against every checked competitor
// regardless of size, which is the exact bug this whole fix was supposed
// to close. Now calls the dedicated getDomainRating() per domain
// alongside getDomainMetrics -- doubles this sub-check's Ahrefs call
// count (was N+1, now 2*(N+1) for N tracked competitors checked), worth
// knowing for Ahrefs plan/quota purposes, but this is fixing already-
// intended, already-shipped behavior, not new scope.
async function fetchKeywordCountMetrics(client, activeCompetitors, { apiKey, maxCompetitors = MAX_KEYWORD_METRIC_COMPETITORS } = {}) {
  ...
  const [clientMetrics, clientRating, competitorResults] = await Promise.all([
    getDomainMetrics(client.domain, { apiKey }),
    getDomainRating(client.domain, { apiKey }),
    Promise.all(competitorsToCheck.map(async c => {
      const [metrics, rating] = await Promise.all([
        getDomainMetrics(c.domain, { apiKey }),
        getDomainRating(c.domain, { apiKey })
      ])
      // error here is deliberately ONLY the metrics (org_keywords) call's
      // error, matching competitive-position-checker.js's existing
      // `failedCompetitors = competitors.filter(c => c.error)` contract --
      // a domain-rating-only failure shouldn't get this competitor
      // excluded from the keyword-count average entirely, it should just
      // mean selectScaleComparableCompetitors falls back gracefully for
      // that one competitor (see its own "withoutDr" handling).
      return { domain: c.domain, orgKeywords: metrics.orgKeywords, domainRating: rating.domainRating, error: metrics.error }
    }))
  ])

  return {
    clientOrgKeywords: clientMetrics.orgKeywords,
    clientDomainRating: clientRating.domainRating,
    // Deliberately NOT merged with clientRating.error -- clientError gates
    // competitive-position-checker.js's whole keyword-count sub-check as
    // "not_verified" (see its `else if (km.clientError)` branch). A failed
    // domain-rating call shouldn't blank out a perfectly good org_keywords
    // count; it should just mean clientDomainRating is null and
    // selectScaleComparableCompetitors falls back to its non-scale-aware
    // behavior, same as before this fix existed. Surfaced separately
    // (non-blocking) as clientDomainRatingError for visibility/debugging.
    clientError: clientMetrics.error,
    clientDomainRatingError: clientRating.error,
...
module.exports = { detectAndSyncCompetitors, detectFromAiCitations, detectFromAhrefs, pruneStaleCompetitors, deactivateStaleCompetitors, fetchKeywordCountMetrics, fetchMissingKeywordOpportunities, selectScaleComparableCompetitors, MAX_SCALE_COMPARABLE_COMPETITORS, MIN_AI_CITATION_OCCURRENCES, STALE_AFTER_DAYS }
