...
async function fetchKeywordCountMetrics(client, activeCompetitors, { apiKey, maxCompetitors = MAX_KEYWORD_METRIC_COMPETITORS } = {}) {
  ...
  const [clientResult, competitorResults] = await Promise.all([
    ...
    Promise.all(competitorsToCheck.map(c =>
      getDomainMetrics(c.domain, { apiKey }).then(r => ({ domain: c.domain, orgKeywords: r.orgKeywords, error: r.error }))
    ))
  ])
...
// total organic keyword count -- reusing keywordMetrics.competitors
// (already fetched by fetchKeywordCountMetrics, no extra Ahrefs call spent
// just to rank them) so the domains diffed against are the strongest
// content performers among the ones already checked, not an arbitrary
// subset. Then pulls each domain's actual organic-keywords LIST (not just
// the total count fetchKeywordCountMetrics already has) via Ahrefs'
// organic-keywords report -- the same report business-profile.js already
// uses for AI-visibility prompt generation, reused here for a different
// purpose. Returns null when keywordMetrics itself is null (not enough
// competitors to bother) or none of its competitor entries have usable
// keyword-count data to rank by.
//
...
async function fetchMissingKeywordOpportunities(client, keywordMetrics, { apiKey, maxCompetitors = MAX_KEYWORD_OPPORTUNITY_COMPETITORS, sampleSize = KEYWORD_OPPORTUNITY_SAMPLE_SIZE } = {}) {
  ...
  const ranked = (keywordMetrics.competitors || [])
...
module.exports = { detectAndSyncCompetitors, detectFromAiCitations, detectFromAhrefs, fetchKeywordCountMetrics, fetchMissingKeywordOpportunities }
