...
const { fetchSitemapPages, MAX_SITEMAP_PAGES_LISTED } = require('./sitemapDiscovery')
...
async function run() {
  ...
  // 10. the listed-pages cap is still honored -- MAX_SITEMAP_PAGES_LISTED
  // trims the returned `pages` array while `count` still reports the real total.
  {
    const many = Array.from({ length: MAX_SITEMAP_PAGES_LISTED + 5 }, (_, i) => `<url><loc>https://example.com/page-${i}/</loc></url>`).join('')
    const fetcher = mockFetcher({ 'https://example.com/sitemap.xml': `<urlset>${many}</urlset>` })
    const result = await fetchSitemapPages('https://example.com/sitemap.xml', { fetcher })
    assert.strictEqual(result.count, MAX_SITEMAP_PAGES_LISTED + 5)
    assert.strictEqual(result.pages.length, MAX_SITEMAP_PAGES_LISTED)
    assert.strictEqual(result.truncated, true)
    log('TEST 10 (the real total page count is preserved even when the returned page list is trimmed to MAX_SITEMAP_PAGES_LISTED) PASSED')
  }
