...
// removeStaleClientProfileFieldItem(...) -- Phase 1b completion fix
// (2026-08-17): the third possible resolution of an open recommendation,
// alongside accept and dismiss above. Added specifically for Phase 1b's
// multi-value list reconciliation: when a CONFIRMED or OVERRIDDEN list
// item (e.g. one geography market of several) is no longer detected by a
// later classification pass, it must not be silently dropped from current
// state (the pinning invariant already prevents that automatically -- see
// fn_write_client_profile_field_system's conflict branch) NOR left
// confirmed forever just because nothing else ever clears it. This lets
// an AM explicitly resolve that recommendation as "yes, actually remove
// it": the slot clears back to null/unconfirmed (not deleted -- item_index
// stays reserved so a future real re-detection of the same position still
// works normally) and the OLD value is preserved in history, exactly like
// every other transition here -- "history preserves it, current state
// does not," never the reverse. Distinct from acceptClientProfileRecommendation
// above, which always sets the current value to whatever was recommended
// -- not appropriate here, since what's "recommended" for a removal is a
// sentinel placeholder (see clientIndustryIntelligence.js's
// STALE_ITEM_REMOVAL_SENTINEL), never real data to persist.
async function removeStaleClientProfileFieldItem({ recommendationId }) {
  const supabase = getSupabaseServerClient()
  const { data, error } = await supabase.rpc('fn_remove_stale_client_profile_field_item', {
    p_recommendation_id: recommendationId
  })
  if (error) throw error
  return data
