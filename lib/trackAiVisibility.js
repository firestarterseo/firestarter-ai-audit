...
async function trackClientAiVisibility(client) {
  ...
  const rows = engineResults.map(er => ({
    ...
    // bestListCited (added 2026-08-13): whether this response used
    // best/top/recommended-list framing alongside a third-party citation --
    // persisted rather than re-derived later so ai-visibility-checker.js
    // doesn't need its own copy of the responseSnippet-based heuristic
    // fully in sync with the live checker's; it still falls back to
    // re-deriving this from responseSnippet for rows logged before this
    // field existed (see rowCitationTier()).
    raw: {
      prompt: er.prompt,
      ...
      bestListCited: er.ok ? !!er.bestListCited : false
    }
  }))
