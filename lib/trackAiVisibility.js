...
async function trackClientAiVisibility(client) {
  ...
  const rows = engineResults.map(er => ({
    ...
    // responseSnippet/sourceUrls (added 2026-08-11): the same transparency
    // data the snapshot path now captures (see ai-visibility-snapshot-
    // checker.js) -- lets ai-visibility-checker.js's dashboard breakdown
    // show what each engine actually said for a tracked client, not just
    // the mentioned/cited booleans.
    raw: { prompt: er.prompt, responseSnippet: er.ok ? (er.responseSnippet || null) : null, sourceUrls: er.ok ? (er.sourceUrls || []) : [] }
  }))
