...
// the competitor has. ---
{
  ...
  const clientPageWeak = {
    ...
    title: 'Patio Doors Denver - JDI Windows', h1: 'Altius Windows and Doors', fetchFailed: false,
  ...
  assert.ok(deficitTypes.includes('location_not_structural'), 'Littleton is mentioned once in body copy but has no heading built around it')
...
// --- computeRelevanceGap: location_absent (city never mentioned at all) ---
{
  const promptText = 'sliding patio doors denver'
  ...
  const clientPage = { url: 'https://jdiwindows.com/doors/', title: 'Sliding Patio Doors Denver', h1: 'Sliding Patio Doors Denver', fetchFailed: false, html: '<h1>Sliding Patio Doors Denver</h1>' }
  const client = { domain: 'jdiwindows.com', city: 'Littleton', region: 'Colorado' }
  const result = computeRelevanceGap({ promptText, competitorPage, clientPage, client })
  assert.ok(result.deficits.some(d => d.type === 'location_absent'), 'client\'s own registered city/region never appears anywhere on the page')
  log('PASS computeRelevanceGap (location_absent when client\'s own city/region is never mentioned)')
}
