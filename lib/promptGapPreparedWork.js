...
const NEW_PAGE_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      title_h1_reason: { type: 'string', description: 'One sentence: which specific gap_evidence/recommended_action this title/H1 addresses.' },
      ...
      sections: {
        ...
        items: {
          ...
          properties: {
            ...
            addresses_gap: { type: 'string', description: 'One sentence: which specific piece of gap_evidence or recommended_action this section is intended to close -- e.g. "closes the missing commercial-intent page finding" or "matches the competitor\'s word-count depth on custom window materials."' }
          },
          required: ['heading_level', 'heading', 'content_html', 'addresses_gap']
        }
      },
    ...
    required: ['page_title', 'h1', 'title_h1_reason', 'target_url_slug', 'meta_description', 'angle', 'sections', 'internal_linking_suggestions', 'proof_requirements', 'schema_recommendations']
  }
}

const NO_PAGE_SYSTEM_PROMPT = `You are a senior local SEO content strategist writing a real, ready-to-publish content brief for a client business. You are given the REAL prompts real people ask AI assistants where a specific tracked competitor is cited and this client is not, plus the gap-analysis engine's own real evidence about why. Ground every recommendation in that real evidence. Never invent facts about the client (credentials, awards, client counts, pricing) that weren't given to you. Write copy substantive enough to be a real first draft, not a stub. For title_h1_reason and every section's addresses_gap, name the SPECIFIC piece of gap_evidence or recommended_action it closes -- never a generic "improves SEO" justification. Also propose internal_linking_suggestions (real, plausible existing pages on this site that should connect to this one), proof_requirements (real-world proof -- photos, testimonials, license/certification numbers -- the client must actually supply before the page's claims are honest; never invent the proof itself), and schema_recommendations (standard schema.org type names for this page, for the existing Schema Wizard to pick up separately -- do not write JSON-LD yourself).`
...
const EXISTING_PAGE_PROPOSAL_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      headings_to_change: {
        type: 'array',
        description: 'H2/H3 changes ONLY -- the page\'s H1 change, if any, belongs exclusively in proposed_h1 above; never repeat it here. These are needed so the page has a SECTION actually built around the missing topic(s), not just a title/H1 mention. Use current_page.headings to decide between renaming an existing heading vs. adding a new one.',
        items: {
          type: 'object',
          properties: {
