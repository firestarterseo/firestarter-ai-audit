...
async function testRequestVerificationRejectsBeforeExecutionCompletes() {
  ...
  await assert.rejects(() => requestVerification('opp-1', { actor: 'am' }), /executed.*human_claimed_complete/)
...
async function testRequestVerificationSucceedsAfterExecutedOrHumanCompleted() {
  ...
  // human_claimed_complete (2026-09-11): additive, more honestly-named
  // status -- identical eligibility to human_completed, never a rename.
  reset({ id: 'opp-6', client_id: 'c1', execution_status: 'human_claimed_complete', execution_capability: 'red', verification_status: 'failed_verification' })
  const result3 = await requestVerification('opp-6', { actor: 'am' })
  assert.strictEqual(result3.verificationStatus, 'ready_to_verify')
  assert.strictEqual(updateCalls.length, 1, 'a prior failed_verification must never block re-requesting verification')

  log('TEST (requestVerification succeeds and writes verification_status=ready_to_verify + a verification_requested history row for "executed", "human_completed", and "human_claimed_complete" opportunities, including a retry after a prior failed_verification) PASSED')
}
