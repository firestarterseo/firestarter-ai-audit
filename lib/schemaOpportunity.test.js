...
// 2026-09 production-verification pass: OPPORTUNITY_TYPE must be one of
// the exact values production's `opportunities_type_check` CHECK
// constraint allows (confirmed via read-only introspection of the real
// firestarter-ai-audit Supabase project) -- 'schema_page_diagnostic' (the
// prior value) is NOT in that list and was guaranteed to fail every
// insert with a Postgres 23514 check-violation. 'schema_fix' is both a
// production-legal value AND the value lib/opportunityLifecycle.test.js's
// own Schema fixtures have used all along.
const PRODUCTION_LEGAL_OPPORTUNITY_TYPES = [
  'content_brief', 'citation_target', 'schema_fix', 'technical_fix', 'entity_verification', 'brand_association_gap'
]

test('OPPORTUNITY_TYPE is exactly "schema_fix" -- the production-supported type value, not the never-legal "schema_page_diagnostic"', () => {
  assert.strictEqual(OPPORTUNITY_TYPE, 'schema_fix')
  assert.ok(PRODUCTION_LEGAL_OPPORTUNITY_TYPES.includes(OPPORTUNITY_TYPE), 'OPPORTUNITY_TYPE must be a value opportunities_type_check actually allows in production')
})

test('OWNING_PILLAR is the exact approved value ("schema_structure" is a real pillar id)', () => {
...
async function main() {
  ...
  await atest('qualifySchemaPageOpportunity calls qualifyOpportunity with opportunityType schema_fix, owningPillar/originatingPillar schema_structure, the stable fingerprint, executionCapability red, for ACTION_REQUIRED', async () => {
    await withMockedQualifyOpportunity(
      ...
      async (mod, calls) => {
        ...
        assert.strictEqual(input.opportunityType, 'schema_fix', 'must send the production-legal type value, never schema_page_diagnostic')
  ...
  await atest('qualifySchemaPageOpportunity calls qualifyOpportunity with opportunityType schema_fix and originatingPillar schema_structure for IMPROVEMENT_AVAILABLE (a real Recommended gap)', async () => {
    await withMockedQualifyOpportunity(
      async () => ({ opportunityId: 'opp-456', action: 'created' }),
      async (mod, calls) => {
        const analysis = analysisOf('IMPROVEMENT_AVAILABLE', { recommendedChecks: [{ id: 'about_page_subtype', status: 'fail', tier: 'recommended', evidence: 'generic WebPage type used' }] })
        const result = await mod.qualifySchemaPageOpportunity({ clientId: 'c1', path: '/about/', pageUrl: 'https://example.com/about/', analysis })
        assert.strictEqual(result.eligible, true)
        assert.strictEqual(calls.length, 1)
        const input = calls[0]
        assert.strictEqual(input.opportunityType, 'schema_fix')
        assert.strictEqual(input.owningPillar, 'schema_structure')
        assert.strictEqual(input.originatingPillar, 'schema_structure')
        assert.strictEqual(input.executionCapability, 'red')
      }
