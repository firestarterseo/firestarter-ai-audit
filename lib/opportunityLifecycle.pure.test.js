...
function testVerificationRequiresGenuinelyCompletedExecutionOrHandoff() {
  ...
  assert.match(notStarted.reason, /executed.*human_claimed_complete/)
  ...
  assert.strictEqual(humanCompleted.ok, true, 'RED opportunities become verification-eligible once a human confirms the handed-off work is actually done (legacy status name)')

  // human_claimed_complete (2026-09-11): an additive, more honestly-named
  // status -- same claim, same eligibility, never a rename/removal of
  // human_completed (Schema Wizard/Source & Citation Wizard keep using
  // that name unchanged).
  const humanClaimedComplete = validateExecutionGate({ execution_status: 'human_claimed_complete', execution_capability: 'red' }, 'verify')
  assert.strictEqual(humanClaimedComplete.ok, true, 'RED opportunities become verification-eligible once a human claims the handed-off work is live -- a claim, identically eligible, never itself success')

  // A prior failed_verification must never block a retry -- the gate only
  // cares about execution_status, not verification_status.
  const retryAfterFailedVerification = validateExecutionGate({ execution_status: 'human_claimed_complete', execution_capability: 'red', verification_status: 'failed_verification' }, 'verify')
  assert.strictEqual(retryAfterFailedVerification.ok, true, 'a prior failed_verification must never block re-requesting verification once the AM has corrected the live page -- no re-handoff, no plan regeneration required')

  log('TEST (verification is only reachable after execution genuinely completed -- "executed", "human_completed", or "human_claimed_complete", never before, never after a failure, and a failed verification never blocks a retry) PASSED')
}
