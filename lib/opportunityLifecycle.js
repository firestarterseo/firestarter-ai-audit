...
const EXECUTION_STATUSES = [
  ...
  'handoff_requested', 'handed_off', 'human_completed', 'human_claimed_complete'
]
...
function validateExecutionGate(row, action) {
  ...
  if (action === 'verify') {
    // 'human_claimed_complete' (2026-09-11) is accepted identically to
    // 'human_completed' -- same claim, clearer name (see
    // recordHumanClaimedComplete's header). Also, deliberately, this gate
    // does NOT care about the current verification_status -- a prior
    // 'failed_verification' never blocks a retry once the AM has actually
    // fixed the live page; there is no separate "unlock retry" step and
    // never a requirement to regenerate the plan.
    const completedExecutionStatuses = ['executed', 'human_completed', 'human_claimed_complete']
    if (!completedExecutionStatuses.includes(row?.execution_status)) {
      return {
        ok: false,
        reason: `Verification cannot be requested until execution actually completed (execution_status must be "executed" or "human_claimed_complete", was "${row?.execution_status || 'not_started'}"). Call executeOpportunity (GREEN/YELLOW) or recordHumanClaimedComplete (RED, after handoff) first.`
      }
    }
    return { ok: true }
  }
...
// recordHumanClaimedComplete(opportunityId, {notes, actor}) -> (2026-09-11)
// same TASK/EXECUTION-COMPLETION-ONLY semantics as recordHumanCompleted
// above -- added specifically because a real prompt-gap content-execution
// run showed 'human_completed' read, in practice, as "the work is done and
// good" even though the code never treated it that way (status stays
// 'open', nothing closes until a REAL recordVerification('verified') call).
// The word "completed" alone invites that misreading. This is a CLAIM the
// human made ("I've made these changes live"), pending a real re-fetch --
// never itself success. Kept as a SEPARATE, additive execution_status
// value (not a rename of 'human_completed') so Schema Wizard and Source &
// Citation's existing, already-correct manual-execution flows are
// untouched -- only new callers that want the clearer name use this one.
// validateExecutionGate's 'verify' gate (below) accepts EITHER value
// identically, so behavior is unchanged either way -- this is a naming
// fix, not a new rule.
async function recordHumanClaimedComplete(opportunityId, { notes = null, actor = 'am' } = {}) {
  const supabase = getSupabaseServerClient()
  const row = await getOpportunityById(supabase, opportunityId)
  const { error } = await supabase.from(OPPORTUNITY_TABLE).update({
    execution_status: 'human_claimed_complete',
    execution_state: { ...(row.execution_state || {}), completed_notes: notes, completed_at: nowIso() },
    updated_at: nowIso()
  }).eq('id', opportunityId)
  if (error) throw error
  await insertHistory(supabase, { clientId: row.client_id, opportunityId, eventType: 'human_claimed_complete', actor, reason: notes })
  return { opportunityId, executionStatus: 'human_claimed_complete' }
}
...
module.exports = {
  ...
  // execution / handoff
  executeOpportunity, requestHandoff, recordHandoff, recordHumanCompleted, recordHumanClaimedComplete,
  // verification
