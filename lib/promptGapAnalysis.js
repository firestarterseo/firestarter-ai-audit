...
async function persistBrandAssociationEvidence(supabase, { clientId, competitorId, competitorDomain, targetTopic, inspections }) {
  ...
  try {
    // Supabase-js does NOT throw on a query/constraint error -- it resolves
    // with {data, error} -- so this must be checked explicitly, not just
    // wrapped in try/catch, or a real write failure silently vanishes.
    const { error } = await supabase.from(BRAND_ASSOCIATION_TABLE).insert(rows)
    if (error) console.error('[brand_association_evidence] insert failed:', error.message || error)
  } catch (e) {
    console.error('[brand_association_evidence] insert threw:', e.message || e)
