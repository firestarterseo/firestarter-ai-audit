...
// normalizeTokenForm(token) -> a small, deliberately narrow singular/
// plural equivalence (strip one trailing "s" from tokens >=5 chars, so
// short real words like "gas"/"bus" are never mangled). Added 2026-09-11
// after a real regression: JDI's client-page sitemap shortlist (see
// discoverClientPage below) never matched "windows" (the prompt's real
// wording) against "window" (the real page's URL slug
// /denver-custom-window-replacement/), which cost that real commercial
// page a full overlap point against blog-article candidates whose slugs
// happened to contain the literal plural -- excluding a genuinely
// relevant client page from the shortlist entirely. Deliberately NOT
// folded into tokenize()/overlapTerms() themselves (used throughout this
// file, already covered by other tests) -- scoped only to
// fuzzyOverlapTerms/rankShortlistCandidates below, the one place this
// mismatch actually caused a real miss.
function normalizeTokenForm(token) {
  return (token.length >= 5 && token.endsWith('s')) ? token.slice(0, -1) : token
}

// fuzzyOverlapTerms(promptTokens, pageTokens) -> like overlapTerms, but a
// prompt token also counts as matched if ITS singular/plural-normalized
// form matches some page token's normalized form (see normalizeTokenForm).
function fuzzyOverlapTerms(promptTokens, pageTokens) {
  const normalizedPageTokens = new Set([...pageTokens].map(normalizeTokenForm))
  return [...promptTokens].filter(t => pageTokens.has(t) || normalizedPageTokens.has(normalizeTokenForm(t)))
}

// INFORMATIONAL_PAGE_TYPES -- lib/sitemapDiscovery.js#classifyPage types
// that are never a commercial/service destination. Used ONLY as a
// shortlist TIE-BREAK signal below (never a hard filter) -- a genuinely
// well-matching Article can still be shortlisted when nothing else scores
// as high; this only decides which candidate wins when two tie on raw
// token overlap.
const INFORMATIONAL_PAGE_TYPES = new Set(['Article', 'Case Study'])

// rankShortlistCandidates(promptText, pages) -> the PRIORITY-2 sitemap
// shortlist ranking discoverClientPage uses, extracted as its own pure
// function (2026-09-11) so the real JDI regression this fixes -- a
// genuinely relevant commercial page losing its shortlist slot to two
// informational blog articles -- has an independent, no-network regression
// test (see lib/promptGapAnalysis.pure.test.js). Ties on overlap count are
// broken in favor of a non-informational page type; among two informational
// (or two non-informational) candidates tied on overlap, original sitemap
// order is preserved (Array.prototype.sort is stable).
function rankShortlistCandidates(promptText, pages) {
  const promptTokens = tokenize(promptText)
  return (Array.isArray(pages) ? pages : [])
    .map(page => ({ page, overlap: fuzzyOverlapTerms(promptTokens, tokenize(`${page.path || ''} ${page.type || ''}`)) }))
    .filter(s => s.overlap.length > 0)
    .sort((a, b) => {
      if (b.overlap.length !== a.overlap.length) return b.overlap.length - a.overlap.length
      const aInformational = INFORMATIONAL_PAGE_TYPES.has(a.page.type) ? 1 : 0
      const bInformational = INFORMATIONAL_PAGE_TYPES.has(b.page.type) ? 1 : 0
      return aInformational - bInformational
    })
}
...
// (does the PROMPT'S OWN target location -- see extractPromptLocation
// below, NOT unconditionally the client's registered city/region --
// appear in a heading, only in passing body text, or not at all), and
// internal-link support (does this page link to other relevant content the
// competitor's doesn't, scoped to links found on the page itself, never a
// full site audit).
...
// GENERIC_SERVICE_TERMS -- common product/service/marketing words that
// appear in almost any home-services prompt in this vertical and carry no
// location signal on their own. Used ONLY by extractPromptLocation to
// isolate what's left of a prompt once its service/product wording is
// removed -- deliberately small and evolvable, same "small, evidence-
// driven list" discipline as this file's other heuristics (see e.g.
// lib/promptGapPreparedWork.js's GENERIC_H1_WORDS, a sibling list for a
// sibling problem -- kept as an independent literal copy here rather than
// an import, since that module already imports FROM this one and this
// file must never import from it back).
const GENERIC_SERVICE_TERMS = new Set([
  'custom', 'windows', 'window', 'doors', 'door', 'replacement', 'install',
  'installation', 'installer', 'company', 'best', 'top', 'quality', 'service',
  'services', 'repair', 'contractor', 'contractors', 'patio', 'sliding',
  'glass', 'home', 'homes', 'construction', 'new', 'local'
])

// extractPromptLocation(promptText, client) -> {tokens, source} | null.
// FIX (2026-09-11, live regression): computeRelevanceGap previously always
// required the CLIENT'S OWN registered city/region to appear structurally
// on the page, regardless of what the prompt actually asked about -- for
// JDI (registered city: Littleton), the prompt "custom windows denver" was
// incorrectly judged to have a relevance gap because Littleton wasn't in a
// heading, even though the page has a real, structural "...in Denver"
// H2 and Denver -- not Littleton -- is what the prompt is actually about.
// This function derives the location the PROMPT itself expresses:
//   1. Strip known generic service/product words (GENERIC_SERVICE_TERMS)
//      and stopwords (tokenize() already drops the latter) from the
//      prompt's tokens. What's left is presumed to name the prompt's
//      target location, e.g. "custom windows denver" -> "denver";
//      "replacement windows littleton co" -> "littleton" ("co" is
//      already dropped by tokenize()'s length>=3 floor).
//   2. If any of those remaining tokens match the client's OWN registered
//      city/region, that's the target location AND it's genuinely the
//      prompt's own location (satisfies requirement 3's first exception) --
//      e.g. "replacement windows littleton co" for a Littleton-registered
//      client.
//   3. Otherwise, the remaining tokens themselves are the target location,
//      REGARDLESS of whether they match the client's registered city --
//      e.g. "denver" for a Littleton-registered client. The client's
//      registered city is never substituted in here.
// Returns null when no non-generic token remains (no clear location in
// the prompt at all) -- callers must treat client city/region as
// supporting context only in that case, never a mandatory requirement
// (requirement 4).
function extractPromptLocation(promptText, client) {
  const promptTokens = [...tokenize(promptText)]
  const candidateTokens = promptTokens.filter(t => !GENERIC_SERVICE_TERMS.has(t))
  if (candidateTokens.length === 0) return null

  const clientLocationTokens = tokenize([client?.city, client?.region].filter(Boolean).join(' '))
  const namesClientLocation = candidateTokens.filter(t => clientLocationTokens.has(t))
  if (namesClientLocation.length > 0) {
    return { tokens: namesClientLocation, source: 'client_location_named_in_prompt' }
  }
  return { tokens: candidateTokens, source: 'prompt_derived' }
}
...
function computeRelevanceGap({ promptText, competitorPage, clientPage, client, clientPageDiscovery } = {}) {
  ...
  // 5. Service/location specificity -- evaluated against the LOCATION THE
  // PROMPT ITSELF EXPRESSES (extractPromptLocation), not unconditionally
  // the client's registered city/region (see that function's header for
  // the live JDI regression this fixes). Checked against headings vs.
  // body-only vs. absent entirely, same structural-vs-passing distinction
  // as before.
  let locationSpecificity = null
  const promptLocation = extractPromptLocation(promptText, client)
  const clientStructuralTokens = new Set([...clientTitleTokens, ...clientHeadingTokens])

  if (promptLocation) {
    // The prompt clearly names a target location -- this is a real,
    // evaluable requirement (requirement 3: mandatory only when it's the
    // prompt's own location, whether or not that happens to match the
    // client's registered city).
    const targetLocationTokens = new Set(promptLocation.tokens)
    const headingLocationMatches = overlapTerms(targetLocationTokens, clientStructuralTokens)
    const bodyOnlyLocationMatches = overlapTerms(targetLocationTokens, clientBodyTokens).filter(t => !headingLocationMatches.includes(t))
    const locationLabel = promptLocation.tokens.join(', ')
    locationSpecificity = { targetLocation: locationLabel, targetLocationSource: promptLocation.source, clientLocationInHeadings: headingLocationMatches, clientLocationInBodyOnly: bodyOnlyLocationMatches }
    evidence.push(`Prompt's target location (${locationLabel}) in this page's headings: ${headingLocationMatches.join(', ') || '(not at all)'}; mentioned only in body copy: ${bodyOnlyLocationMatches.join(', ') || 'none'}.`)
    if (headingLocationMatches.length === 0 && bodyOnlyLocationMatches.length > 0) {
      deficits.push({ type: 'location_not_structural', terms: bodyOnlyLocationMatches, detail: `The page mentions ${bodyOnlyLocationMatches.join(', ')} (this prompt's target location) in passing body copy but has no heading built around it -- the page isn't structured around this location.` })
    } else if (headingLocationMatches.length === 0 && bodyOnlyLocationMatches.length === 0) {
      deficits.push({ type: 'location_absent', terms: [...targetLocationTokens], detail: `The page never mentions this prompt's target location (${locationLabel}) anywhere.` })
    }
  } else {
    // No clear location in the prompt -- the client's registered city/
    // region is recorded as supporting context ONLY (requirement 4): real,
    // informational evidence, but it can never produce a deficit here.
    const clientLocationTokens = tokenize([client?.city, client?.region].filter(Boolean).join(' '))
    if (clientLocationTokens.size > 0) {
      ...
      locationSpecificity = { targetLocation: null, supportingContext: locationLabel, clientLocationInHeadings: headingLocationMatches, clientLocationInBodyOnly: bodyOnlyLocationMatches }
      evidence.push(`This prompt names no clear location -- client's own registered location (${locationLabel}) shown as supporting context only (not a relevance requirement): in headings: ${headingLocationMatches.join(', ') || '(not at all)'}; in body copy only: ${bodyOnlyLocationMatches.join(', ') || 'none'}.`)
    }
  }
...
async function discoverClientPage(promptText, client, competitorPage, { cloroApiKey, fetcher, anthropicApiKey } = {}) {
  ...
  const shortlist = rankShortlistCandidates(promptText, sitemapResult.pages).slice(0, MAX_SHORTLIST_CANDIDATES)
...
module.exports = {
  ...
  // pure logic
  tokenize, overlapTerms, fuzzyOverlapTerms, rankShortlistCandidates,
  extractPromptLocation,
