...
// "exists but isn't ranking" and "couldn't check"; clientPageDiscovery's
// intent/pageType/matchConfidence/selectionReason (see discoverClientPage,
// backed by lib/pageIntentMatch.js) explain WHY that specific page was
// selected, not just that it was.
function computeContentGap({ competitorPage, clientPage, clientPageDiscovery }) {
  ...
  if (!clientPage) {
    ...
    // literally no candidate shared any keywords, or candidates existed but
    // NONE actually satisfied the classified intent (the JDI how-to-guide
    // case) -- these are honestly worded differently.
    const reasonLine = clientPageDiscovery && clientPageDiscovery.noPageReason === 'no_candidate_satisfies_intent'
      ? `${clientPageDiscovery.candidatesConsidered || 0} client page(s) shared keywords with this prompt, but none actually satisfy the classified intent (${clientPageDiscovery.intent}) -- no appropriate client page exists for this prompt, even though keyword-overlapping pages exist.`
...
//   url, source, status, intent, intentReason, intentSource, pageType,
//   matchConfidence, selectionReason, competitorIntentMatch, llmUsed,
//   noPageReason?, candidatesConsidered?
// }
//
// Gathers EVERY plausible candidate (the live-SERP result, if any, plus up
// to MAX_INTENT_CANDIDATES sitemap pages ranked by keyword overlap), then
// asks lib/pageIntentMatch.js which ONE (if any) actually satisfies the
// prompt's classified intent -- selection is intent-driven, not just
// overlap-driven. Only the chosen candidate is ever fetched for the deep
// content/relevance/technical comparisons; every other candidate stays a
// cheap path/type lookup, never a live page fetch.
async function discoverClientPage(promptText, client, competitorPage, { cloroApiKey, fetcher, anthropicApiKey } = {}) {
  ...
  const promptTokens = tokenize(promptText)
  const scoredSitemapPages = (sitemapResult && Array.isArray(sitemapResult.pages) ? sitemapResult.pages : [])
    .map(page => ({ page, overlap: overlapTerms(promptTokens, tokenize(`${page.path || ''} ${page.type || ''}`)) }))
    .filter(s => s.overlap.length > 0)
    .sort((a, b) => b.overlap.length - a.overlap.length)
    .slice(0, MAX_INTENT_CANDIDATES)
    .map(s => ({ url: s.page.url, path: s.page.path, type: s.page.type, source: 'sitemap' }))

  const candidates = []
  if (serp.url) candidates.push({ url: serp.url, path: safePathFromUrl(serp.url), source: 'serp' })
  for (const c of scoredSitemapPages) {
    if (!candidates.some(x => x.url === c.url)) candidates.push(c)
  }

  if (candidates.length === 0) {
    if (!siteUrl || !sitemapResult) return { url: null, source: null, status: 'unable_to_identify' }
    return { url: null, source: null, status: 'no_relevant_page', noPageReason: 'no_candidates', candidatesConsidered: 0 }
  }
  ...
  if (evalResult.bestIndex == null) {
    return {
      url: null, source: null, status: 'no_relevant_page', noPageReason: 'no_candidate_satisfies_intent',
  ...
  const chosen = candidates[evalResult.bestIndex]
  const chosenEval = evalResult.candidateEvaluations.find(e => e.index === evalResult.bestIndex) || {}
  return {
    url: chosen.url,
    source: chosen.source,
    status: chosen.source === 'serp' ? 'relevant_page_ranks' : 'relevant_page_exists_not_ranking',
    intent: evalResult.intent, intentReason: evalResult.intentReason, intentSource: evalResult.intentSource,
...
async function analyzePromptGap(clientId, promptText, {
  ...
  const clientPage = clientPageDiscovery.url ? await fetchPage(clientPageDiscovery.url, { fetcher }) : null
