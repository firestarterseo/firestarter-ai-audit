...
const { toggleQueuedPath, resolveOpenPath, queuedCount, resolveActiveWorkItem } = require('./schemaPageSelection')
...
function run() {
  ...
  // (e.g. a different client's page) could accidentally read or write.
  {
    ...
    assert.deepStrictEqual(exportedNames, ['queuedCount', 'resolveActiveWorkItem', 'resolveOpenPath', 'toggleQueuedPath'])
  ...
  // 5. resolveActiveWorkItem -- 2026-09-04d WORKFLOW CORRECTION. Covers the
  // exact live bug: Steps 4/5/6 must resolve which work item (home vs a
  // specific page) is active from the SAME effectiveOpenPath/homepage-path
  // comparison every other step already uses -- never a second notion of
  // "current page."
  {
    // The homepage is open -> type 'home'.
    let item = resolveActiveWorkItem({ effectiveOpenPath: '/', homepagePath: '/' })
    assert.deepStrictEqual(item, { type: 'home', path: '/' })

    // A real page is open -> type 'page', same path echoed back.
    item = resolveActiveWorkItem({ effectiveOpenPath: '/about/', homepagePath: '/' })
    assert.deepStrictEqual(item, { type: 'page', path: '/about/' })

    // Nothing open at all (e.g. zero candidate pages) -> honest null, never
    // a guessed default.
    item = resolveActiveWorkItem({ effectiveOpenPath: null, homepagePath: '/' })
    assert.deepStrictEqual(item, { type: null, path: null })

    // No homepage entry discovered yet -- any open path is still a real
    // page-level work item, never mistaken for home.
    item = resolveActiveWorkItem({ effectiveOpenPath: '/about/', homepagePath: null })
    assert.deepStrictEqual(item, { type: 'page', path: '/about/' })

    // The homepage's REAL discovered path is honored even when it isn't
    // the literal string '/' -- same discipline as resolveOpenPath/
    // homepageEntry elsewhere in this project (never a hardcoded '/').
    item = resolveActiveWorkItem({ effectiveOpenPath: '/home', homepagePath: '/home' })
    assert.deepStrictEqual(item, { type: 'home', path: '/home' })

    // Sequential advancement: once /about/ is done and the next
    // recommended/open page becomes /contact/, this must independently
    // resolve to /contact/ as its own page-level item -- no state carried
    // over from /about/.
    item = resolveActiveWorkItem({ effectiveOpenPath: '/contact/', homepagePath: '/' })
    assert.deepStrictEqual(item, { type: 'page', path: '/contact/' })

    log('TEST 5 (resolveActiveWorkItem resolves home vs. a specific page from the same effectiveOpenPath/homepage-path state every other step uses, with an honest null when nothing is open) PASSED')
  }
