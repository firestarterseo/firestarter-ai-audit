...
//     failureDetail, currentSchema: [], applicable: [], missingOrInvalid: [],
//     notApplicable: [], actionableGap: null }
// `actionableGap: null` (not false) on failure is deliberate -- "we
// couldn't check" must never collapse into "nothing to fix," which is
// what a page-lifecycle caller uses to keep a failed-fetch page from being
// wrongly marked NO_ACTION_NEEDED (see lib/schemaPageLifecycle.js).
async function analyzePage({ path, page = {}, siteUrl, fetcher } = {}) {
  ...
  if (!url) {
    return {
      path, classification, fetchState: 'failed',
      failureCategory: 'invalid_url', failureDetail: 'Could not resolve an absolute URL for this page.',
      currentSchema: [], applicable: [], missingOrInvalid: [], notApplicable: [], actionableGap: null
    }
  }
  ...
  if (fetchResult.fetchState !== 'success') {
    return {
      path, classification, fetchState: 'failed',
      failureCategory: fetchResult.failureCategory, failureDetail: fetchResult.failureDetail,
      currentSchema: [], applicable: [], missingOrInvalid: [], notApplicable: [], actionableGap: null
    }
  }

  const { byType, schemaNames, failed } = parseJsonLd(fetchResult.html)
  const checkResult = runPageTypeChecks(classification.type, { byType, schemaNames, failed })
