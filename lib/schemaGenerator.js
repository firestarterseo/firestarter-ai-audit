...
// extractContactHints(html, opts) -> { phone, address, sameAs }
...
// costs nothing extra to check. Three sources, most trustworthy first:
...
//      node with a real telephone/address but be missing sameAs or
//      BreadcrumbList, and that partial data is exactly what the generator
//      needs, real and already-structured.
...
// Returns nulls/empty array for anything not found -- callers merge this
// into whatever the client record already has, never overwrite a value a
// strategist deliberately set.
function extractContactHints(html, { domain } = {}) {
  if (!html) return { phone: null, address: null, sameAs: [] }
  ...
  return { phone, address, sameAs: Array.from(sameAs).slice(0, 8) }
}
