...
async function generateSchemaWithHints(client) {
  // Phase 2A (2026-09-01): migrated to lib/webPageFetch.js -- this was a
  // genuinely safe migration since any fetch failure already degraded to
  // an empty string with no scoring/grade implications either way, exactly
  // preserved below. (A bare try/catch was previously the only guard; now
  // any non-success fetchState is treated the same as the old catch block.)
  let homepageHtml = ''
  const fetchResult = await fetchWebPage(client.url, { headers: { 'User-Agent': 'FirestarterAIAudit/1.0' } })
  if (fetchResult.fetchState === 'success') homepageHtml = fetchResult.html
