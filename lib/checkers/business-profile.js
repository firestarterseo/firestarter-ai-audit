...
const { getOrganicKeywords, topPromptCandidates } = require('./ahrefs')
...
// enrichProfileWithAhrefs(profile, opts) -> Promise<profile>
// Adds `ahrefsTerms` -- real, volume-validated keywords this business's
// own domain already ranks for -- when an AHREFS_API_KEY is configured.
// This is the only genuinely async step in this module (a live network
// call); every caller that builds a profile in order to auto-generate
// prompts (runAudit.js, the test-prompts API route, the tracking cron job)
// already runs inside an async function, so this is just one more await
// in that same chain. Skipped entirely (profile returned unchanged) when
// no API key is configured yet, or when the domain has no organic
// rankings to return -- generatePromptCandidates falls through to
// title/meta/schema-guessed terms in either case, no different from any
// other missing data source in this project.
async function enrichProfileWithAhrefs(profile, { apiKey } = {}) {
  if (!profile || !profile.domain || !apiKey) return profile
  const rows = await getOrganicKeywords(profile.domain, { apiKey })
  const ahrefsTerms = topPromptCandidates(rows)
  return ahrefsTerms.length ? { ...profile, ahrefsTerms } : profile
}
...
function generatePromptCandidates(profile) {
  ...
  const { name, category, city, region, titleTerms, metaPhrase, ahrefsTerms } = profile
  ...
  //  1. Ahrefs organic keywords (when available) -- real, ranking- and
  //     volume-validated terms this exact domain already earns traffic
  //     from. Not a guess at all.
  //  2. Title/meta-derived phrasing -- real copy the business wrote about
  //     itself, but not validated against actual search demand.
  //  3. Guessed "best <category> in <city>" combos below -- the fallback
  //     of last resort, used only when neither of the above found
  //     anything (e.g. a brand-new domain with no rankings yet, no
  //     Ahrefs key configured, and a thin/JS-only homepage with no useful
  //     title or meta description).
  if (Array.isArray(ahrefsTerms)) candidates.push(...ahrefsTerms)
...
module.exports = { extractBusinessProfile, generatePrompts, generatePromptCandidates, enrichProfileWithAhrefs, extractTitleAndMeta, CATEGORY_LABELS }
