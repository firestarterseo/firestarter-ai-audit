...
// countSitemapPages(url, fetcher) -> Promise<number|null>
// Purely additive, non-scored signal (2026-08-16) -- added specifically to
// make the Diagnosis step's "Site-wide" stat pill (see workflow-mockup.html's
// #pane-schema, stat-pill data-pill="sitewide") a real number instead of the
// illustrative "5 pages" the mockup shows. technical-checker.js already
// fetches sitemap.xml for its own robots/sitemap check -- this is the same
// fetch, just also counted here, since Schema & Structure needed its own
// real page count and re-fetching a small XML file is cheap. NEVER affects
// score/grade (same "additive evidence, not a scored check" convention as
// content-checker.js's competitor gap analysis) -- returns null (not 0) on
...
async function countSitemapPages(url, fetcher = fetch) {
  if (!url) return null
  try {
    const res = await fetcher(`${normalizeUrl(url)}/sitemap.xml`)
...
async function checkSchemaAndStructure(htmlOrUrl, { url = null, fetcher = fetch } = {}) {
  ...
  // Real page count for the Diagnosis step's "Site-wide" stat pill -- see
  // countSitemapPages's header comment. `url` is only passed when the
  // caller has it (runAudit.js does); falls back to null (rendered as an
  // honest "not checked" in the UI, never a fake number) when it isn't.
  const sitemapPageCount = await countSitemapPages(url, fetcher)
