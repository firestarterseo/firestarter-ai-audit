...
async function checkSchemaAndStructure(htmlOrUrl, { url = null, fetcher = fetch } = {}) {
  ...
  return {
    ...
    _raw: {
      ...
      // lib/sitemapDiscovery.js) + a cheap real URL-pattern classification
      // (Home/Service/Location/Article/Case Study/About/Contact/
      // Uncategorized) -- backs SchemaWizard.js's Page coverage step. null
      // (not []) when the sitemap fetch failed or predates this field, same
      // "absent vs. confirmed empty" contract sitemapPageCount already uses.
      sitemapPages
    }
  }
}
