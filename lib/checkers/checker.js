...
// Phase 2A (2026-09-01): the real fetch resolveHtml performs (its URL-fetch
// branch, confirmed dead code in production -- every real caller here
// always passes pre-fetched HTML, never a raw URL) goes through the shared
// lib/webPageFetch.js primitive instead of a bare fetch() call. headers: {}
// is passed explicitly to preserve this file's exact prior request shape
// (no custom headers were sent before this migration) rather than picking
// up lib/webPageFetch.js's own default User-Agent.
const { fetchWebPage } = require('../webPageFetch')
// 2026-09-02 sitemap/page-discovery fix: real sitemap fetching, index/urlset
// distinction, recursive child-sitemap traversal, and page classification
// now all live in the shared lib/sitemapDiscovery.js module -- see that
// file's header for why this is a new shared module rather than a reuse of
// technical-checker.js's own (private, count-only, non-recursive) sitemap
// check. This checker no longer parses sitemap XML itself at all.
...
// countSitemapPages(url, fetcher) -> Promise<{ count: number, pages: Array<{path, type, sourceSitemap}> } | null>
// Purely additive, non-scored signal (2026-08-16, extended 2026-08-17,
// fixed 2026-09-02) -- added specifically to make the Diagnosis step's
// "Site-wide" stat pill (see workflow-mockup.html's #pane-schema,
// stat-pill data-pill="sitewide") a real number, and to let the Page
// coverage step (step 2) list real page paths. Delegates entirely to
// lib/sitemapDiscovery.js#fetchSitemapPages, which correctly tells a
// <sitemapindex> (child sitemap references -- never pages) apart from a
// <urlset> (real pages), recurses into child/nested sitemaps with a bounded
// depth guard, dedupes pages across sitemaps, and excludes non-HTML asset
// URLs. Before this fix, this function ran one flat regex against the
// fetched sitemap.xml body with no such distinction -- a client whose root
// sitemap was an index (the common Yoast shape: sitemap_index.xml ->
// post-sitemap.xml, page-sitemap.xml, ...) had every child-sitemap URL
// treated and displayed as if it were a real page. NEVER affects
// score/grade (same "additive evidence, not a scored check" convention as
// content-checker.js's competitor gap analysis) -- returns null (not a
// zero-length result) on any failure, so the UI can tell "confirmed zero
// pages" apart from "couldn't check." Only classifies WHAT KIND of page
// each real URL looks like -- it does NOT claim to know whether that
// page's own schema passes or fails; only the homepage is actually run
// through this checker's 7 checks today.
async function countSitemapPages(url, fetcher = fetch) {
  if (!url) return null
  try {
    const result = await fetchSitemapPages(`${normalizeUrl(url)}/sitemap.xml`, { fetcher })
...
async function checkSchemaAndStructure(htmlOrUrl, { url = null, fetcher = fetch } = {}) {
  ...
  return {
    ...
    _raw: {
      ...
      sitemapPageCount,
      // Real page URLs (never sitemap XML files -- see
      // lib/sitemapDiscovery.js), up to MAX_SITEMAP_CANDIDATES_DISCOVERED,
      // each with a transparent classification record (type, source,
      // confidence, reason -- Home/Service/Location/Article/Case Study/
      // About/Contact/Utility-Legal/Other) -- backs SchemaWizard.js's Page
      // coverage step, which further prioritizes/recommends from this list
      // via lib/schemaPagePriority.js. null (not []) when the sitemap fetch
      // failed or predates this field, same "absent vs. confirmed empty"
      // contract sitemapPageCount already uses.
      sitemapPages
    }
  }
}
