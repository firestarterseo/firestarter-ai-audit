...
// business's own name.
//
// `best_position_url` added 2026-09-02 (Page Leverage Intelligence Audit
// follow-up) -- confirmed against Ahrefs' own docs
// (https://docs.ahrefs.com/api/reference/site-explorer/get-organic-keywords,
// checked directly for this change, not assumed) as a real, existing select
// field on this SAME report: "The ranking URL in organic search results."
// This is the one field that turns an already-fetched, already-paid-for
// domain-aggregate keyword row into page-attributable evidence -- ZERO new
// Ahrefs API calls, zero new endpoints, same request shape, just one more
// field in an existing `select` list. See getOrganicKeywords() below for how
// it's surfaced (as `rankingUrl`), and lib/pageSearchFootprint.js for the
// pure module that joins it to sitemap pages. Every existing caller of
// getOrganicKeywords() destructures only the fields it already knew about
// (keyword/volume/position/branded/local) -- none of them enumerate an
// object's keys or do a strict shape/equality check -- so this addition is
// purely additive and changes no existing scoring/aggregation behavior.
const SELECT_FIELDS = ['keyword', 'volume', 'best_position', 'best_position_url', 'is_branded', 'is_local']
...
// getOrganicKeywords(domain, opts) -> Promise<Array<{keyword, volume, position, rankingUrl, branded, local}>>
...
// result).
//
// `rankingUrl` (added 2026-09-02, see SELECT_FIELDS comment above) is
// Ahrefs' own `best_position_url` field -- the specific page on this domain
// that ranks for this keyword at `position`. It is `null`, honestly, when
// Ahrefs doesn't return one for a given row (never fabricated). This field
// is NEW -- every pre-existing caller of this function already only reads
// keyword/volume/position/branded/local, so it arrives as an extra, unused
// field for them, changing nothing about their behavior.
  ...
  try {
    ...
    return rows
      ...
      .map(r => ({
        ...
        rankingUrl: typeof r.best_position_url === 'string' && r.best_position_url ? r.best_position_url : null,
