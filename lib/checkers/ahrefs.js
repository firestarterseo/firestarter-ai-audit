...
// caught the actual root cause: this endpoint requires a `country` param
// (verified against Ahrefs' own docs, 2026-08-14 -- a 2-letter ISO
// 3166-1 alpha-2 code), which the OTHER Ahrefs endpoints this tool calls
// (organic-keywords, backlinks-stats) do NOT require -- every prior call
// here was failing with HTTP 400 "missing argument 'country'" and
// silently reading as "no competitor data" instead. Every client in this
// tool is US-based today, so 'US' is a reasonable default when the
// caller doesn't supply one; this becomes wrong the day a non-US client
// is added and should be revisited then (ideally derived from the
// client's own region, once that's mapped to a country code somewhere).
...
module.exports = { getOrganicKeywords, topPromptCandidates, getBacklinksStats, getOrganicCompetitors }
