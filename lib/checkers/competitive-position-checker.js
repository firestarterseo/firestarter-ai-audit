...
//   opts.ahrefsError: null on a clean Ahrefs call (even one that
//     legitimately found zero competitors), or { status, message }
//     describing why the call itself failed -- surfaced directly in the
//     evidence instead of a generic "no data" message when present, so a
//     real API/permission problem doesn't read identically to an expected
//     "these two signals just don't overlap yet" gap.
function checkCompetitivePosition(client, competitors, { aiVisibilityRows = [], ahrefsCompetitorData = [], ahrefsError = null } = {}) {
  const evidence = []
  const findings = []
  ...
  if (comparableAhrefs.length > 0) {
    ...
    if (ahrefsError) {
      // A real, distinguishable failure -- not "legitimately no data."
      const statusPart = ahrefsError.status ? `HTTP ${ahrefsError.status}` : 'call failed'
      evidence.push(`Ahrefs organic-competitors call failed (${statusPart}: ${ahrefsError.message}) -- this is an API/config problem, not a case of the two signals simply not overlapping. Check AHREFS_API_KEY and whether the account's plan includes the organic-competitors report (other Ahrefs calls in this tool, like backlinks-stats, can work fine while this specific report is restricted).`)
    } else if (!ahrefsCompetitorData || ahrefsCompetitorData.length === 0) {
      evidence.push('Ahrefs returned no organic-competitor data for this domain at all this run (a clean, error-free response) -- this domain likely doesn\'t yet rank for enough organic keywords for Ahrefs to compute competitors from.')
    } else {
  ...
  const keywordPossible = comparableAhrefs.length > 0 ? 40 : 0
