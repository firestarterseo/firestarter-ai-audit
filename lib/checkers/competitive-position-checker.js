...
function checkCompetitivePosition(client, competitors, { aiVisibilityRows = [], ahrefsCompetitorData = [], ahrefsError = null, keywordMetrics = null, keywordOpportunityData = null } = {}) {
  ...
  // --- Per-competitor breakdown (added 2026-08-17) -----------------------
  // workflow-mockup.html's #pane-comp Diagnosis step shows a ranked
  // "compared against N tracked competitors" disclosure, each row with that
  // competitor's own real keyword count and how many times it was cited
  // instead of this client. This checker already computes both numbers
  // internally -- perCompetitorTally (ties/losses per domain, built in the
  // AI-citation loop above) and km.competitors (per-competitor org_keywords
  // from the same Ahrefs metrics call the keyword-count sub-check already
  // makes) -- but neither was ever persisted to _raw, so no honest UI could
  // show this list without either a second live API round-trip or making
  // the numbers up. No new data collection here, just keeping what this run
  // already fetched instead of discarding it.
  const competitorBreakdown = activeCompetitors.map(c => {
    const tally = perCompetitorTally[c.domain] || { ties: 0, losses: 0 }
    const kwEntry = (km?.competitors || []).find(k => k.domain === c.domain)
    return {
      domain: c.domain,
      name: c.name || null,
      orgKeywords: (kwEntry && typeof kwEntry.orgKeywords === 'number' && !kwEntry.error) ? kwEntry.orgKeywords : null,
      // "cited instead" -- every AI-visibility run where this specific
      // competitor's domain showed up in the citations, whether or not the
      // client was ALSO mentioned that run (a tie) or wasn't (a loss to
      // this competitor specifically). Ties and losses are exactly the two
      // branches that populate perCompetitorTally above.
      citedCount: tally.ties + tally.losses
    }
  }).sort((a, b) => {
    // Ranked by keyword count when Ahrefs verified it this run (matches the
    // mockup's own "highest keywords first" ordering), falling back to
    // citation count for any competitor Ahrefs couldn't verify -- never
    // silently drops a competitor just because one of the two numbers is
    // unavailable.
    if (a.orgKeywords !== null && b.orgKeywords !== null) return b.orgKeywords - a.orgKeywords
    if (a.orgKeywords !== null) return -1
    if (b.orgKeywords !== null) return 1
  ...
  return {
    ...
    _raw: {
      ...
      // Added 2026-08-17 -- see competitorBreakdown's own comment above for
      // why this exists and where each number comes from.
      competitorBreakdown,
      // Added 2026-08-15 so runAudit.js can pass this client's own Ahrefs
      // domain rating into lib/keywordRelevance.js's LLM refinement call
      // without a second Ahrefs round-trip -- already fetched by
      // fetchKeywordCountMetrics for the scale-matching fix above.
      clientDomainRating: km?.clientDomainRating ?? null,
      wins,
      ties,
      losses,
      excluded,
