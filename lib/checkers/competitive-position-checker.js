...
function checkCompetitivePosition(client, competitors, { aiVisibilityRows = [], ahrefsCompetitorData = [] } = {}) {
  ...
  if (comparableAhrefs.length > 0) {
    ...
    evidence.push('No Ahrefs keyword-overlap data available for any tracked competitor -- keyword-count component not verified this run (requires AHREFS_API_KEY, and only covers competitors Ahrefs itself surfaced).')
  }
