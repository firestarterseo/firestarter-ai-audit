...
async function checkAiVisibilitySnapshot(profile, prompts, { caller = defaultCloroCaller, engines = DEFAULT_ENGINES, apiKey, now = new Date() } = {}) {
  ...
  const engineResults = await Promise.all(tasks.map(async ({ promptText, engine }) => {
    ...
    if (!raw || raw.success === false || !raw.result) {
      return { prompt: promptText, engine, ok: false, error: (raw && raw.error) || 'no result' }
    }
    ...
    return { prompt: promptText, engine, ok: true, mentioned, cited, sentiment }
  }))
  ...
  return {
    ...
    recommendation,
    evidence,
    _raw: { profile, promptsUsed: prompts, engineResults, mentionShare, citationShare, sentimentShare, coverageShare }
  }
}
