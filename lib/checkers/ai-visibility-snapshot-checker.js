...
// mention never earns citation credit here even with a source present --
// e.g. a lawsuit writeup citing the business isn't an endorsement just
// because it's third-party (sentiment is already penalized on its own via
// the separate sentiment score component, so this isn't double-counting a
// bad outcome, just declining to reward it twice as a "citation" too).
function citationTier({ mentioned, ownCited, thirdPartyCited, authorityCited, sentiment }) {
  if (!mentioned) return 0
  if (sentiment === 'negative') return 0
  if (authorityCited) return 1
  if (thirdPartyCited) return 0.75
  if (ownCited) return 0.5
  return 0
}
...
async function checkAiVisibilitySnapshot(profile, prompts, { caller = defaultCloroCaller, engines = DEFAULT_ENGINES, apiKey, now = new Date() } = {}) {
  ...
  const engineResults = await Promise.all(tasks.map(async ({ promptText, engine }) => {
    ...
    // self/third-party/authority distinction is what actually drives
    // scoring now (see citationTier above).
    const cited = ownCited || thirdPartyCited
    const sentiment = mentioned ? guessSentiment(combinedText) : null
    const citation = citationTier({ mentioned, ownCited, thirdPartyCited, authorityCited, sentiment })
  ...
  // all" share. A client whose mentions are all self-cited tops out at 50%
  // of this component; one whose mentions are backed by recognized
  // third-party authority sources can reach 100%.
  ...
  const authorityMentions = mentionedList.filter(e => e.authorityCited).length
  const thirdPartyOnlyMentions = mentionedList.filter(e => e.thirdPartyCited && !e.authorityCited).length
  const ownOnlyMentions = mentionedList.filter(e => e.ownCited && !e.thirdPartyCited).length
  const noCitationMentions = mentionedList.filter(e => !e.ownCited && !e.thirdPartyCited).length
  if (mentionedList.length > 0) {
    evidence.push(`Citation mix among mentions: ${authorityMentions} from a recognized authority source, ${thirdPartyOnlyMentions} from another independent third party, ${ownOnlyMentions} citing only the business's own site, ${noCitationMentions} mentioned by name with no citation at all.`)
  }
  ...
  } else if (ownOnlyMentions > 0 && authorityMentions === 0 && thirdPartyOnlyMentions === 0) {
    findings.push(`Cited in ${ownOnlyMentions} mention(s), but only via the business's own site -- no independent third-party source is backing up these mentions yet.`)
