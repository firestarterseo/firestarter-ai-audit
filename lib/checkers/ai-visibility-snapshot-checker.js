...
// perplexity, and copilot -- each using the exact prompt this module
// auto-generates for Denver Tax Advisor. The chatgpt/google pass caught two
// real bugs the documented-shape assumption missed:
...
const DEFAULT_ENGINES = ['chatgpt', 'gemini', 'perplexity', 'copilot', 'google']
...
async function checkAiVisibilitySnapshot(profile, prompts, { caller = defaultCloroCaller, engines = DEFAULT_ENGINES, apiKey, now = new Date() } = {}) {
  ...
  // Only the primary (first) prompt is used per engine for this v1 --
  // keeps the check simple to verify. See business-profile.js for the
  // note on extending to multiple prompt variations later.
  const promptText = prompts[0]
  evidence.push(`Live snapshot as of ${now.toISOString().slice(0, 10)} using the prompt: "${promptText}" (auto-generated from this page's structured data, not hand-picked).`)

  // These 5 calls are independent (different engines, same prompt) -- run
  // them concurrently, not one at a time. Sequentially, 5 live API calls
  // (each easily 5-20s) can add up to well over a minute and blow past any
  // reasonable request timeout; concurrently, total wall-clock is roughly
  // the slowest single call, not the sum of all five. This is what was
  // actually behind the "Run audit now" button appearing to freeze earlier.
  const settled = await Promise.all(engines.map(async engine => {
    ...
    if (!raw || raw.success === false || !raw.result) {
      return { engine, ok: false, error: (raw && raw.error) || 'no result' }
    }
    ...
    return { engine, ok: true, mentioned, cited, sentiment }
  }))
  engineResults.push(...settled)
  ...
  engineResults.forEach(e => {
    if (!e.ok) {
      evidence.push(`${e.engine}: call failed (${e.error}) -- excluded from scoring.`)
    } else {
      evidence.push(`${e.engine}: ${e.mentioned ? 'mentioned' : 'not mentioned'}${e.mentioned ? `, ${e.cited ? 'cited/sourced' : 'not cited'}, sentiment ${e.sentiment || 'undetermined'}` : ''}.`)
    }
  })

  const mentionShare = responded.length > 0 ? mentionedList.length / responded.length : 0
  const citationShare = mentionedList.length > 0 ? citedList.length / mentionedList.length : 0
  const sentimentShare = sentimentDetermined.length > 0 ? positiveList.length / sentimentDetermined.length : 0
  const coverageShare = totalQueried > 0 ? responded.length / totalQueried : 0
  ...
  if (responded.length < totalQueried) {
    findings.push(`${totalQueried - responded.length} of ${totalQueried} engine call(s) failed -- this snapshot is based on partial coverage.`)
  ...
  if (mentionShare < 0.5) {
    findings.push(`Not mentioned by ${Math.round((1 - mentionShare) * 100)}% of engines queried in this single live check.`)
  ...
  const finding = findings.length > 0
    ? findings.join(' ')
    : `Mentioned and cited across the engines queried in this single live snapshot as of ${now.toISOString().slice(0, 10)}.`
  ...
  return {
    ...
    recommendation,
    evidence,
    _raw: { profile, promptUsed: promptText, engineResults, mentionShare, citationShare, sentimentShare, coverageShare }
  }
}

module.exports = { checkAiVisibilitySnapshot, defaultCloroCaller, scoreToGrade, DEFAULT_ENGINES }
