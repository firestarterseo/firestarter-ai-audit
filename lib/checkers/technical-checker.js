...
async function checkPageSpeed(url, fetcher, apiKey) {
  ...
  if (!res.ok) {
    ...
    issues.push({
      severity: 'info',
      message: `PageSpeed Insights request failed (${res.status || res.error}).`,
      why: 'A failed API call is a data gap, not a finding about the site itself.',
      recommendation: 'Confirm the API key is valid and the PageSpeed Insights API is enabled, then re-run.'
    })
