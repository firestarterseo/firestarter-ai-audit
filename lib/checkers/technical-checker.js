...
// collectPerformanceOpportunities(lhr) -> [{title, savingsMs}]
...
function collectPerformanceOpportunities(lhr, { max = 4 } = {}) {
  ...
  for (const ref of auditRefs) {
    ...
    opportunities.push({ title: audit.title, savingsMs: Math.round(savingsMs) })
  }
  return opportunities.sort((a, b) => b.savingsMs - a.savingsMs).slice(0, max)
}

// collectFailingAudits(lhr, categoryId) -> [{title, weight}]
...
function collectFailingAudits(lhr, categoryId, { max = 4 } = {}) {
  ...
  for (const ref of auditRefs) {
    ...
    failing.push({ title: audit.title, weight: ref.weight })
  }
