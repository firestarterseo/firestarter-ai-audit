...
// recordHandoff -> recordHumanCompleted, all via the existing generic
// app/api/clients/[id]/opportunities/[opportunityId]/lifecycle route) --
// the AM applies the approved change in WordPress by hand, then confirms
// completion, then this module's verify step re-fetches the live page for
// real. canAutoPublishToWordPress() is the one place that states this
...
async function buildExecutionReview(clientId, opportunityId, { client, fetcher } = {}) {
  ...
  if (actionType === 'improve_existing_page' || actionType === 'expand_existing_page') {
    ...
    if (fetchResult && fetchResult.fetchState === 'success' && fetchResult.html) {
      livePage = { title: extractTitleAndH1(fetchResult.html).title, h1: extractTitleAndH1(fetchResult.html).h1, wordCount: htmlToWordCount(fetchResult.html) }
    }
...
// requestVerification, rejects until execution_status is 'executed' or
// 'human_completed' -- same enforcement Schema's verify-work route relies
// on). Always re-reads the APPROVED prepared-work row fresh from the DB,
